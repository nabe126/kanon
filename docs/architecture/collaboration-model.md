...
## 📄 3. コードレビュー提出ポリシー (Repository Review Policy)

エージェント（AGY）が人間にコードレビューや変更点の提示を要求する際、文脈の肥大化とノイズを防ぐため、以下のルールに従って情報を提出しなければなりません。

### 基本的な提出パッケージ
1. **`git diff --stat`**: 変更規模と対象ファイルを俯瞰するための統計情報。
2. **変更・追加ファイルの一覧**: 物理的なファイルパスと、それぞれの新規/変更ステータス。
3. **実行テストログ**: 単体テスト（pytest等）の成否サマリー。
4. **実機未検証リスト**: 安全性のために「まだ実機で動かしていないこと」の明示。
5. **前提となる環境・構成情報**: ポート、パス、コンテナ名等の仮定条件。

### ソースコード全文の提示制限
エージェントは、**コード全文をチャット画面に大量に出力してはなりません。** 全文を提示してよいのは以下の場合に限定されます：
* ユーザー（人間）から「全文を提示してください」と明示的に要求された場合。
* 対象コードが1ファイルあたり **100行未満** である場合。
* レビュー対象が単一のファイルのみである場合。

上記以外の場合、エージェントは **`git diff`（コード差分）** または **モジュールごとの設計サマリー（役割説明）** のみを提示し、コード全文の読み取りはユーザーが必要に応じてエディタリンク等を介してローカルファイル（`file://` パス）から行う運用とします。
