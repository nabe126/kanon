...
### P5: 運用負荷の削減と知見のコード化 (リポジトリによる記憶)
* **設計アプローチ**:
  - **コマンド実行の簡略化 (Makefile 運用ツール化)**:
    - `docker compose` コマンド実行時に毎回 `--env-file` オプションを指定する手間を省き、エラーを防ぐため、ルートの `Makefile` に実機運用のためのターゲット群（`up`, `down`, `logs`）を追加。
  - **トラブルシューティングのツール化**:
    - 本日の検証で遭遇した「ポート5000競合」「AppArmorによる停止ブロック」等のトラブルについて、ワンコマンドで検知・解決できる `Makefile` ターゲット（`fix-port`, `fix-apparmor`, `fix-perms` 等）を整備。
  - **トラブルシューティングガイドの文書化**:
    - 運用で得られた知見を `docs/guides/troubleshooting.md` に集約し、人間や他のエージェントが同じ手順で迷わないように「リポジトリに記憶」させる。
...
### Step 0: 運用ラッパーツール(Makefile)とトラブルシューティング文書の整備 (本ターン最優先対象)
* **作業内容**:
  - ルートの `Makefile` の更新（実機操作ラッパーコマンド、トラブルシューティング用コマンドの追加）。
  - `docs/guides/troubleshooting.md` の新規作成（今回の検証知見の集約）。
* **検証内容**: Mac および実機側で `make help` 等が期待通り表示され、定義が正常に機能すること。

### Step 1: 設計の合意と P1 テスト容易化のためのリファクタリング
* **作業内容**: 
