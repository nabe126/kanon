...
### 💻 開発・検証の役割分離 (Environment Separation)

Kanon は「用途分離」「役割固定」の原則に基づき、物理環境の役割を以下のように分離して非同期開発を行っています。
* **開発環境 (M4 Mac)**: コードの編集、設計書・ドキュメント更新、L1単体テスト（pytest）の実行、およびコミット・プッシュを担当。AIエージェント（AGY）の主たる稼働エリア。
* **検証環境 (GPD WIN 3 / Ubuntu 26.04)**: Docker コンテナのビルド・起動検証、実機でのポート監視、ホスト側モニター（`monitor.py`）の動作試験などの実機検証（L2/L3）を担当。エージェントからの直接接続（SSH）は行わず、人間が手動で検証を実行する。
* **中継地点 (GitHub)**: 双方の修正を同期・マージするための媒介。
...
## 💻 7. CLI REPL 起動方法 (CLI REPL Interactive Mode)

開発環境において、Discord等の外部チャットを介さずに、直接ターミナル上でエージェントとの対話や、Memory / ASEP の動作をテストするための REPL を提供しています。
